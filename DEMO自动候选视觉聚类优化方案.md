...
面向读者的当前架构总览见 [README 海报](README.md#架构)；本文保留配置、算法约束和验收顺序等实现级细节。
...
自动分割页按源图组织默认折叠的结果块，摘要只显示原始、保留和过滤数量，展开后再展示该图全部保留候选。视觉聚类页在顶部统一解释同图约束、`clu_…`、`prop_…`和CLS最低/平均/最高值，各聚类卡不重复相同原理说明。
