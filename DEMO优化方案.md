...
> 本文是后续开发规划，不是当前已实现能力说明。当前正式实验事实仍以 `PROGRESS.md` 为准。本文所述 DINOv3、单次 Qwen、视觉指纹和新对象摘要结构均尚未在服务器正式验证。

## 0. 开始开发前先准备什么

### 0.1 先确认服务器与磁盘

后续开发仍在现有 AutoDL 项目环境中完成：
...
开发时在 `config/default.yaml` 增加一组明确配置，路径由配置读取，不散落在代码里：
...
当前第二轮 Qwen 还承担候选完整性和附着部件判断。删除后，这部分由以下证据共同承担：
...
1. 用全局指纹与所有历史视角计算余弦相似度；
2. 按对象聚合，取每个对象的最佳历史视角作为粗排结果；
3. 对排名靠前的对象计算局部 patch 对应分数；
4. 结合全局分数、局部分数、第一名与第二名差距，得到视觉身份结论；
5. 阈值只从后续同类实例数据确定，不用现有三实例结果假装完成标定。
...
新实现使用新的空白记忆库进行实验，不从现有 v0.20.3 记忆库猜测生成 DINOv3 指纹或新对象摘要。当前完整记忆库继续作为历史实验依据和 Web 展示证据。数据库 schema 升级时应明确标记旧库只读，不编写静默回填或自动重建路径。
