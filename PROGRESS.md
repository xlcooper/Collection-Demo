...
- **当前问题**：正确 Conda 环境中的 Qwen 尝试尚未进入推理，Transformers 在已有本地权重时仍发起 Hugging Face 在线检查并遇到断网
- **下一动作**：拉取 v0.3.3，使用默认本地缓存模式重跑 Qwen 冒烟
...
| M0-C Qwen 单模型冒烟 | 🟡 | 8B FP8 加载成功，输出可解析的约定 JSON，并记录耗时/显存 | 权重已下载；错误 Conda 环境问题已纠正；正确环境尝试被在线配置检查中断，尚未进入推理 | 使用 v0.3.3 默认本地缓存模式重跑 |
