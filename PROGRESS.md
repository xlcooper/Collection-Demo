...
- **下一动作**：拉取 v0.3.6，运行统一冒烟入口并生成三份报告及 Qwen 吞吐指标
...
| M0-B SAM3 单模型冒烟 | ✅ | 真实加载 checkpoint，并输出至少一个 proposal 和 mask | `cup` 得到 1 个 proposal，score 0.8867；加载 9.167 秒、推理 0.407 秒、峰值显存 5087.16 MiB | 单提示链路完成；全量策略留到 M2 |
...
1. SAM3 的 BF16 链路已跑通：`cup` 得到 1 个 proposal 和 mask，score 0.8867；加载约 9.2 秒、推理约 0.4 秒、峰值显存约 5.0 GiB。
...
4. Qwen 的 9.875 秒包含图像编码和逐 token 生成，明显慢于 SAM 的单次前向，但当前没有 token 数和设备放置，暂不能判为性能异常；v0.3.6 在统一复测中补齐吞吐证据。
5. 本次 JSON 结构完整，但整张场景图输出了 `coffee/latte`，没有严格聚焦物理容器类别。M0 只验证运行和结构；M3 必须使用 SAM crop/mask 与对象限定提示验证语义质量。
6. generation flags 警告未影响确定性生成和 JSON 结果，当前不作为阻塞项。
7. 系统 `nvcc` 11.8 只影响可选源码扩展；当前 MVP 不编译这些扩展，因此不是阻塞项。

## 现在执行

服务器拉取 v0.3.6 后，运行统一入口：
