"""Thin adapter for class-agnostic SAM3 point-grid candidates."""
...
class Sam3Adapter:
    """Load SAM3 once and generate class-agnostic point-grid candidates."""
    ...
    def predict(self, image: Image.Image) -> Sam3Prediction:
        """Generate one best candidate for every positive point in the grid."""
    ...
    def _extract_point_candidates(
        ...
        *,
        ...
        mask_array = self._to_cpu_array(masks)
        score_array = self._to_cpu_array(scores)
        ...
        for point_offset in range(expected_count):
            ...
            bbox = self._mask_bbox(mask) or (0.0, 0.0, 0.0, 0.0)
