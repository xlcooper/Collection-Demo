...
```mermaid
flowchart LR
    A["场景图片"] --> B["SAM3：16×16 自动点网格候选"]
    B --> C["脚本：面积、重叠、包含与数量过滤"]
    C --> D["DINOv3：视觉指纹与跨图聚类"]
    D --> E["DINOv3：与历史对象视角比较"]
    E --> F["Qwen3-VL：结合聚类与历史证据完成语义审查"]
    F --> G["一致性决定与 SQLite 对象记忆"]
```

三种模型按 `SAM3 → DINOv3 → Qwen3-VL` 顺序加载和释放，避免联合驻留。SAM3 先处理全部唯一新图；DINOv3 为保留候选生成指纹，并只在不同源图之间按 CLS 相似度形成聚类；Qwen 随后按聚类批次阅读代表视角接触表和已有对象文字摘要。只有聚类语义判断与历史视觉证据满足提交规则时，系统才新建对象或迭代原有摘要。
...
- 按图片折叠查看 SAM3 保留候选，并通过页面术语说明读懂 DINOv3 聚类编号、成员候选和 CLS 相似度；
