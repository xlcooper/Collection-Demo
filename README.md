...
- 首轮输出必须逐图覆盖精确 `source_id`。中文物体名、场景摘要和理由用于阅读与审计，唯一可能用于 SAM3 查询的是英文小写名词短语 `sam_text_prompt`，执行时会原样传入；报告中的 `prompt_detection_counts` 只列出已完成并记录检测数的查询，中途失败时不能把未记录条目描述成确定未发送。该字段最长64字符，先确定完整物体的稳定基础类别，再按需保留至多一个清晰且有助检索的前置修饰词，例如 `water bottle` 或 `transparent water bottle`；透明瓶不能仅因透明而改写成 `drink cup`。v5 提示不再生成 `with/and` 特征尾句；解析器仅为避免历史整批失败而继续宽容旧的单物体表达，`or` 形式的备选提示仍不接受。中英文类别一致性目前由模型侧自检并在 Web 中供人工审计，不是固定词表校验。
...
- 提供单用户 Web 实验台；每次运行可新建独立空白记忆库，也可选择一份完整的已有记忆继续加入新观测，并可整库删除或清空旧结果，刷新页面或重启服务后仍恢复上次选择。同一时间只允许一个实验进程，运行期间锁定输入和记忆库管理。页面按输入登记、首轮 Qwen、SAM3、第二轮 Qwen 和报告写盘的真实工作单元显示进度、墙钟时间与中间证据，不按预估耗时伪造进度。
- Web 对空白库自动执行全新 Demo 覆盖验收；续写已有库执行同一端到端流程，但不重复要求本轮必须同时制造新对象、已有对象匹配和重复图等首轮覆盖条件。
- Web 端的 SQLite 可视化以“长期对象卡 + 跨视角观测时间线”为主，并提供 `source → proposal → decision → object` 候选血缘表；流程与数据检查状态和需要人工复核的物体识别质量分开呈现。
...
├─ input/                  # 固定 Demo 输入
├─ memory/                 # 兼容的默认记忆库
└─ memory_<内部编号>/      # Web 新建的独立记忆库，可有多份
   ├─ memory.sqlite
   ├─ sources/
   ├─ proposals/
   ├─ objects/
...
每个记忆库都包含同样的 `memory.sqlite`、`sources/`、`proposals/`、`objects/`、`raw_responses/` 和 `run_reports/`。SQLite 保存运行、源图、候选、对象、观测和决策的结构化索引；图像资产使用相对路径与数据库关联，因此一个记忆库必须作为完整单元迁移、提交或删除。仓库中的默认目录可以不存在，首次运行时会自动创建；Web 新库使用服务器生成的安全内部编号，页面名称不会成为服务器路径。

正式实验可将完整记忆库连同最新外部报告一起保留，作为可迁移、可审计的输入—输出证据。只有全部相关 source 和候选均已完成的记忆库才允许在 Web 中继续加入新图片；失败、不完整或不可读的库只供复核和整库删除。并排比较应新建独立空白库，避免运行归属、已有对象和哈希状态互相污染。
...
浏览器打开对应的本机隧道地址或 AutoDL 提供的 HTTPS 自定义服务地址即可；不要把带 Basic 认证的原始 HTTP 端口直接暴露到公网。Web 页面固定调用正式入口，只提交服务器生成的记忆库内部编号，不接受浏览器传入任意服务器路径、模型参数或命令。运行日志与进度事件保存在本地忽略的 `temp/web_runs/`；所选完整记忆库及最新 `environment/run_report.json` 进入 Git 管理。
